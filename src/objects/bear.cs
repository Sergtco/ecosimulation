...
class Bear : Animal
{
    ...
    Vector2 hitboxOffset = new Vector2(5, 20);

    public override Vector2 Size { get { return size * scale; } }
    Vector2 size = new Vector2(22, 11);
    ...
    public Bear(Game1 game) : base(game)
    {
        speed = Vector2.Zero;
        chagePosition(1);
    }

    public Bear(Game1 game, Vector2 position) : this(game)
    {
    ...
    {
        sheet = game.Content.Load<GridSheet>("sprites/bear/MiniBear");
        idleAnimation = new Animation(sheet, 0, 0.3f, 4, true);
        walkAnimation = new Animation(sheet, 1, 0.1f, 6, true);
        howlAnimation = new Animation(sheet, 5, 0.4f, 10, false);
        attackAnimation = new Animation(sheet, 4, 0.2f, 5, false);
        deadAnimation = new Animation(sheet, 7, 0.2f, 4, false);
    }
    ...
    public override void changeState(State newState)
    {
        switch (newState)
        {
            case State.Idle:
                ...
                break;
            case State.Walk:
                move(speed: new Vector2(Rand.get(-60, 60), Rand.get(-60, 60)));
            ...
            if ((state != State.Hunt) && (hunger < 75))
            {
                foreach (Animal animal in game.getObjects())
                {
                    if (animal.Type == Type.Herbivore && animal.CurrState != State.Dead && visionCircle.Collides(animal.Hitbox))
                    {
                        ...
                        prey = animal;
                        prey.isPrey(this);
                        move(pos: prey.Position, speed: new Vector2(150, 150));
                    }
